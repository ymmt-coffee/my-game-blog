...
ローカルの安全機能、rclone 1.75.0、Google OAuth、通常remote、crypt remote、rclone設定ファイルのWindowsユーザー保護まで完了している。架空ファイル1件の暗号化アップロード・復元テストと初回バックアップは成功した。日次・月次タスクは登録・有効化済みである。実Discord通知はまだ行っていない。
