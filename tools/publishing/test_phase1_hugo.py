...
class Phase1HugoTests(unittest.TestCase):
    def test_three_article_types_seo_images_and_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            self.assertRegex(play_html, r'data-website-id="?b6809a40-f862-4c3a-b650-da1fe19d0f39"?')
            self.assertRegex(play_html, r'data-domains="?framing-games\.com"?')
            self.assertRegex(play_html, r'data-exclude-search="?true"?')
            self.assertRegex(play_html, r'data-exclude-hash="?true"?')
            self.assertRegex(play_html, r'data-do-not-track="?true"?')
            self.assertIn('window.location.hostname!=="framing-games.com"', play_html)
            self.assertIn('window.location.pathname.startsWith("/")', play_html)
            self.assertIn("https://framing-games.com/", play_html)
